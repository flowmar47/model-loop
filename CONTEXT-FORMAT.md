...
Source: adapted from [claudex-loop](https://github.com/chaseai-yt/claudex-loop) (MIT).
...
Resolution order: `CONTEXT-MAP.md` present → find the context matching the current work (ask if ambiguous); root `CONTEXT.md` only → single context; neither → create a root `CONTEXT.md` lazily.
